...
class Labeling:
    """
    Contains all of the logic to label the data. This class shouldn't be used directly.
    Use functions instead
    """

    def raw_return(prices, logarithmic=False):
        """
        Raw returns labeling method. Most basic form of labeling based on raw return of each observation relative to its previous value.
        ...
        :return:  (pd.Series or pd.DataFrame) Raw returns on market data. User can specify whether returns will be based on simple or logarithmic return, and whether the output will be numerical or categorical.
        """
        
        shifted_prices = prices.shift(1)
        
        if logarithmic:
            r = np.log(prices) - np.log(shifted_prices)
        else:
            r = (prices / shifted_prices) - 1

        l = r.apply(lambda x: -1 if x < 0 else (0 if x == 0 else 1))
        l[0] = -1
    ...
    def get_events(close, t_events, pt_sl, target, min_ret, num_threads, vertical_barrier_times=False, side_prediction=None):
        ...
        first_touch_dates = ea.mp_pandas_obj(func=apply_pt_sl_on_t1,
    ...
    def barrier_touched(out_df, events):
        """
        Adjust the getBins function to return a 0 whenever the vertical barrier is the one touched first.
